...
var OutputFormats = []string{Json, Yaml, Human}
...
func NewCmd(f kcmdutil.Factory, streams genericiooptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().StringVarP(&options.OutputFormat, "output", "o", Default, fmt.Sprintf(`Output format. One of: (%s)`, strings.Join(OutputFormats, ", ")))
...
func (o Output) Print(format string, out io.Writer) (int, error) {
	...
	return 0, fmt.Errorf("requested output format: %s is not supported", format)
}
